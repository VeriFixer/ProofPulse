/* app.js — plain script (no modules). It expects window.generateSpansFragment, window.parseProof, window.sampleLog, window.sourceCode, window.targetFileName, window.getTokensForFile */
...
function bootstrap() {
  const src = window.sourceCode;
  const proofLog = window.sampleLog;
  const fname = window.targetFileName;


  window.proofObj = window.parseProof(proofLog || '');
