...
async function runDafnyAndAppendLog(srcFilePath, opts = {}) {
  ...
  let boogieValue = "/proverOpt:O:smt.core.minimize=true /proverOpt:O:sat.core.minimize=true  /proverOpt:C:proof=true";
  let spawnOpts = { cwd: dir };

  if (opts.forceMinimization) {
    const projectRoot = path.resolve(__dirname, '..');
    const wrapperPath = path.join(projectRoot, 'packages', 'core', 'scripts', 'z3-minimizer-wrapper.sh');
    const minimizerPath = path.join(projectRoot, 'packages', 'core', 'scripts', 'minimize_unsat_core_trace.py');
...
async function runTest(srcFile, opts = {}) {
  ...
  const dafnyRes = await runDafnyAndAppendLog(srcFile, opts);
  if (!dafnyRes.ok) {
